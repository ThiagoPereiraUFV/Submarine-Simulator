...
int ImageLoad(const char *filename, Image *image) {
	...
	image->nrChannels = 3;
    // read the data.
    image->data = stbi_load(filename, &(image->sizeX), &(image->sizeY), &(image->nrChannels),0);
    if (image->data == NULL) {
...
void updateVariables(const GLsizei w, const GLsizei h) {
	...
	view_h = h;
	verticesSea = {
	{-c3*view_h,-c3*view_h,c3*view_h},{-c3*view_h,c3*view_h,c3*view_h},
	{c3*view_h,c3*view_h,c3*view_h},{c3*view_h,-c3*view_h,c3*view_h},
	{-c3*view_h,-c3*view_h,-c3*view_h},{-c3*view_h,c3*view_h,-c3*view_h},
	{c3*view_h,c3*view_h,-c3*view_h},{c3*view_h,-c3*view_h,-c3*view_h}};
	if(!started) {
...
void display() {
	...
	glLoadIdentity();
	gluLookAt(viewer[0],(fp) ? viewer[1] - 5 : viewer[1], (fp) ? viewer[2] - 22 : viewer[2], 	// define posicao do observador
	center[0], (fp) ? center[1] + 5 : center[1]+10, (fp) ? center[2] - 22 : center[2]+2,        // ponto de interesse (foco)
    ...
    gluQuadricTexture(sphere, true); //gera texturas para as superficies quadricas...
	...
	switch(key) {
		...
		case 49 :
			if(l1 && light) {
				glDisable(GL_LIGHT0);
				l1 = false;
				glClearColor((GLfloat)8/(GLfloat)255, (GLfloat)24/(GLfloat)255, (GLfloat)97/(GLfloat)255, 1.0);
			}
			else if(light) {
				glEnable(GL_LIGHT0);
				l1 = true;
...
void init() {
	...
	glutInitWindowPosition((glutGet(GLUT_SCREEN_WIDTH)-WINDOW_WIDTH)/2, (glutGet(GLUT_SCREEN_HEIGHT)-WINDOW_HEIGHT)/2); //posicao da janela
